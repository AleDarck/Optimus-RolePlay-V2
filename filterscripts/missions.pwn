...
public FCNPC_OnVehicleEntryComplete(npcid, vehicleid, seat)
{
	...
	if(Mission[mission][level] == 1)
	{
		...
		EndCutscene(mission);
		// Start the playback
		FCNPC_StartRecordingPlayback(Mission[mission][npcs][1], "mission_1");
	}
	return 1;
}
...
public PlayCutscene(mission)
{
	...
	if(Mission[mission][level] == 0)
	{
 	...
 	{
		// Start the playback
		FCNPC_StartRecordingPlayback(Mission[mission][npcs][1], "mission_2");
		for(new i = 0; i < MAX_MISSION_PLAYERS; i++)
		{
 	...
 	{
		FCNPC_StartRecordingPlayback(Mission[mission][npcs][0], "mission_3");
	}
 	else if(Mission[mission][level] == 5)
 	{
		...
		FCNPC_StartRecordingPlayback(Mission[mission][npcs][0], "mission_4");
